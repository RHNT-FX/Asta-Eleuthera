...
function handleSendMessage() {
  if (!waMessage.value) return
  const waNumber = kontak.value?.whatsapp || '6281234567890'
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
        ...
        <div>
          ...
          <p class="text-sm text-white/50 mb-4">
            Ada pertanyaan untuk pengurus RT? Kirim pesan langsung via WhatsApp.
          </p>
          <form @submit.prevent="handleSendMessage" class="space-y-2">
            <input
              v-model="waMessage"
              type="text"
              placeholder="Halo Pak RT..."
            ...
            <button
              type="submit"
              class="w-full px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 bg-[var(--color-rt-primary)] text-white hover:bg-[var(--color-rt-primary-light)] flex items-center justify-center gap-2"
            >
    ...
    <div class="border-t border-white/5">
      <div class="max-w-7xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
        <p class="text-xs text-white/30">
          © {{ currentYear }} By Asta EleuThera Team
        </p>
      </div>
    </div>
  </footer>
